...
const threshold = (grid: number[][], val: number) => {
  ...
  for(let y = 0; y < grid.length; y++) {
    ...
    for(let x = 0; x < grid[y].length; x++)
      adjusted[y].push(grid[y][x] > val ? true : false);
  }
    
  return adjusted;
}

const perlinNoise = (width: number, height: number, perlinWidth: number, perlinHeight: number) => {
